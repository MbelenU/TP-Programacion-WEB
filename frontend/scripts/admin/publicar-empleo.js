...
document.addEventListener('DOMContentLoaded', async function() {
    guardarPublicacion.addEventListener('click', async function (event) {
        ...
        let response = await fetch(`http://localhost:80/TP-Programacion-WEB/controllers/AdministradorController.php?publicarEmpleo`, {
            method: 'POST',
            headers: {
        ...
        if(response.success) {
            window.location.href = `http://localhost:80/TP-Programacion-WEB/frontend/views/admin-visualizar-publicacion.php?id=${response.body}`;
        } else {
...
async function agregarHabilidad() {
    ...
    const response = await fetch(`http://localhost/TP-Programacion-WEB/controllers/AdministradorController.php?habilidad=${habilidadText}`)
    if(!response.ok) {
...
async function cargarMaterias(idPlanEstudio) {
    let response = await fetch(`/TP-Programacion-WEB/controllers/AdministradorController.php?idPlanEstudio=${idPlanEstudio}`)
    response = await response.json()
...
async function cargarPlanesEstudio(idCarrera) {
    let response = await fetch(`http://localhost/TP-Programacion-WEB/controllers/AdministradorController.php?id_carrera=${idCarrera}`)
    response = await response.json()
